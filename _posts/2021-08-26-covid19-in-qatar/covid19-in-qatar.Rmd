...
table1(~daily_confirmed + acc_confirmed + daily_deaths + acc_deaths + daily_recovered + acc_recovered,
       ...
       caption = )
```
What can be noticed from the table is the extremely low number of total deaths at `r max(qatar_1$acc_deaths)` from a total of `r round_any(max(qatar_1$acc_confirmed), 1000, ceiling)` cases, representing a death rate of `r max(qatar_1$acc_deaths)/round_any(max(qatar_1$acc_confirmed), 1000, ceiling) * 100`%
